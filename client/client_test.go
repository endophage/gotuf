...
func TestRotation(t *testing.T) {
	...
	err = signed.Sign(signer, "root", signedRoot, rootKey, replacementKey)
...
func TestRotationNewSigMissing(t *testing.T) {
	...
	// Sign testRoot with both old and new keys
	signedRoot, err := testRoot.ToSigned()
	err = signed.Sign(signer, "root", signedRoot, rootKey)
	assert.NoError(t, err, "Failed to sign root")
	var origKeySig bool
	var replKeySig bool
	for _, sig := range signedRoot.Signatures {
...
func TestRotationOldSigMissing(t *testing.T) {
	...
	// Sign testRoot with both old and new keys
	signedRoot, err := testRoot.ToSigned()
	err = signed.Sign(signer, "root", signedRoot, replacementKey)
	assert.NoError(t, err, "Failed to sign root")
	var origKeySig bool
	var replKeySig bool
	for _, sig := range signedRoot.Signatures {
